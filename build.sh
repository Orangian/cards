...
intltool mesa gtk-doc gobject-introspection gnome-common polkit dbus-glib go pkg-config libxcb xcb-util pam cairo libglvnd libev ncurses xorg xorg-xauth glibc gcc libx11 libxkbcommon>=0.5.0 libxkbcommon-x11>=0.5.0 --noconfirm --noprogressbar # Install packages we'll need to build
...
cat ${HOME}/runner/cards/purged-releng.list >> ${PROFILE}/packages.x86_64
# Add custom packages to our profile
cat ${HOME}/runner/cards/user-packages.list >> ${PROFILE}/packages.x86_64
# Add AUR packages to our profile
cat ${HOME}/runner/cards/aur-packages.list >> ${PROFILE}/packages.x86_64
